...
elasticsearch:
  hosts:
    - lab-elasticsearch.observability.svc.cluster.local # [3]
  port: 9200                                            # [4]
  security:
    auth:                                               # [5]
      enabled: true
...
Let's review our configurations:
  1. Enable `DEBUG` mode for Kibana instance. This is helpful for troubleshooting if we encounter any errors.
  2. Specify compute resources for our Kibana instance.
  3. Specify our Elasticsearch host name. As we are deploying the Elastic stack within the same cluster, the host name is the fully qualified domain name or [FQDN](https://github.com/thai-nm/my-chirpy-website-resources/blob/main/2024-03-16-elastic-stack-a-complete-guide-to-set-up-a-modern-log-monitoring-system-on-kubernetes/values-elasticsearch.yaml) of the Elasticsearch Service.
  4. Port number that Elasticsearch listens for incoming HTTP requests. With the installation of Elasticsearch above, this is `9200` by default.
  5. Configure security options when Kibana communicates with Elasticsearch. Here we are telling Kibana that Elasticsearch has enabled the security plugin, which requires any clients to have a username and password to communicate. Then we provide the password Secret `elasticsearch-admin` so that Kibana can use and call Elasticsearch API to create `kibana_system` user, who is used for Kibana-Elasticsearch communication later, with existing Secret `kibana-admin`. As a best practice mentioned above, we should not manage secrets in code, so let's create the `kibana-admin` Secret:
  ```bash
  kubectl create secret generic kibana-admin --from-literal=kibana-password=admin --namespace observability
  ```
...
Let's check if Kibana is running well and port-forward it to our machine:
```bash
# Port forward Elasticsearch Service to local machine port
kubectl port-forward -n observability services/lab-kibana 5601:5601

# Open a new terminal. Check Kibana status
kubectl get pod -n observability

# Access Kibana via a browser with URL [http://localhost:5601]
# Username: elastic
# Password (retrieved from Kubernetes Secret `elasticsearch-admin`): admin
```

Here we are, our Kibana:
<img alt="kibana-status" src="../assets/img/2024-03-16-elastic-stack-a-complete-guide-to-set-up-a-modern-log-monitoring-system-on-kubernetes/kibana-status.png">
